...
void	PhoneBook::add_print(const char *mes, std::string *number)
{
...
void	PhoneBook::add_print(const int cont_index)
{
...
int PhoneBook::page_print(std::string str, int str_index, int input_int)
{
	if (str_index == 0)
	{
...
int PhoneBook::page_print(int input_int, int str_index)
{
	if (str_index > 4)
		return (0);
	std::cout<<"Page"<<input_int<<" |"<<this->Conts[input_int - 1].convert(str_index)<<" is "<<this->Conts[input_int - 1].get_info(str_index)<<std::endl;
	return (1);
}
...
void PhoneBook::search() {
	...
	for (int j = 0; j < add_result ; j++)
		for (int i = 0; page_print(this->Conts[j].get_info(i), i, j); i++);
	if (add_count == 0)
	{
	...
	while (1) {
		...
		{
			std::cout<<"---------------------------------------"<<std::endl;
			for (int i = 0; page_print(input[0] - '0', i); i++);
			break;
		}
	}
};
